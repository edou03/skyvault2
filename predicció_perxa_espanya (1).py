...
informes =pd.read_excel('/content/drive/MyDrive/TFG/Biomecanica CAR/Informes/informes_españa.xlsx')
informes.info()
...
# Imputar valors faltants de 'Talla' segons la mediana per gènere
...
null_percent = null_percent[null_percent > 0]  # Mostrem només les que tenen valors nuls
...
* < 5% de valores nulos → Imputación sin mayores problemas.
* 5% – 30% → Se considera imputar si la variable es relevante, aunque se evalúa con más cuidado.
* (> 30%) → Generalmente se descarta, salvo que haya una justificación sólida para imputar (por ejemplo, alto poder predictivo o imputación fiable basada en otras variables).
"""

# Eliminarem les variebles que no ens aporten res a l'estudi.
# Comencem per les que no són predictives.
...
# Vel. horizontal clavada té 66,87% de valors nuls, no el volem a l'estudi.
informes = informes.drop(columns=['Vel. horizontal clavada'], errors='ignore')
informes.info()

# prompt: també vull eliminar:
...
# 🔹 Imputació de 'peso' amb regressió (usant 'Genero' i 'Talla')
...
# 🔹 Nova imputació de 'pole flex' amb Ridge i variables millorades
features_flex = [
...
# 🔹 Imputació de 'Angulo pertiga' i 'Grip width' per grups
...
# 🔹 Imputació de 'Distancia horizontal impulso/agarre'
...
# 🔍 Informació final del dataframe
df.info()
...
"""Ara he de decidir quines variables imputo i en funció de què i quines variables elimino (peso?)"""

#Vull eliminar les següents variables perquè percentatge elevat de Nan:
...
print("🔍 Linear Regression amb Pipeline")
...
print("\n🌲 XGBoost amb Pipeline")
print(f"Mean Squared Error: {mse_xgb:.4f}")
print(f"R-squared: {r2_xgb:.4f}")

# -------------------------------
# 5. NOVA FUNCIÓ DE PREDICCIÓ
# -------------------------------
def predict_height(model_pipeline, input_dict):
    """
    Prediu la height bar a partir de les característiques d'entrada (diccionari).
    """
    input_df = pd.DataFrame([input_dict])
    return model_pipeline.predict(input_df)[0]

# Exemple de predicció (has d'omplir els valors reals)
sample_input = {
    'Genero': 1,
    'Talla': 1.84,
    'peso': 78,
...
print(f"\n🎯 Predicció amb Linear Regression: {altura_pred_lr:.3f} m")
print(f"🎯 Predicció amb XGBoost: {altura_pred_xgb:.3f} m")
...
print("🔁 Linear Regression - R² mitjà (CV):", np.mean(cv_scores_lr))

# Cross-validation amb XGBoost
cv_scores_xgb = cross_val_score(pipeline_xgb, X, y, cv=cv, scoring='r2')
print("🔁 XGBoost - R² mitjà (CV):", np.mean(cv_scores_xgb))
...
plt.title('📊 Influència de cada variable (Linear Regression)')
plt.gca().invert_yaxis()  # La més important a dalt
plt.grid(True, linestyle='--', alpha=0.5)
plt.tight_layout()
plt.show()

# Importància de variables amb XGBoost
# -------------------------------
importances = pipeline_xgb.named_steps['model'].feature_importances_

# Get the feature names used by the pipeline
# 🐛 Fix: Use the stored feature_names_in_ instead of feature_cols
...
# Gràfic
plt.figure(figsize=(10, 6))
plt.barh(importancia_xgb['Variable'], importancia_xgb['Importància'], color='salmon')
plt.xlabel('Importància (feature importance)')
plt.title('🌲 Importància de variables (XGBoost)')
...
# Gràfic circular del pes relatiu
plt.figure(figsize=(8, 8))
plt.pie(coeficients['% Pes relatiu'], labels=coeficients['Variable'],
        autopct='%1.1f%%', startangle=140, colors=plt.cm.tab20.colors)
plt.title('🥧 Distribució del pes relatiu de les variables (Linear Regression)')
plt.axis('equal')  # Assegura forma de cercle
plt.tight_layout()
plt.show()

# 🔮 CEL·LA FINAL PER A LA PREDICCIÓ MANUAL

# Introdueix aquí les teves dades reals:
dades_reals = {
...
# Predicció amb els dos models
pred_lr = predict_height(pipeline_lr, dades_reals)
pred_xgb = predict_height(pipeline_xgb, dades_reals)

print(f"\n🎯 Predicció amb Linear Regression: {pred_lr:.3f} m")
print(f"🎯 Predicció amb XGBoost: {pred_xgb:.3f} m")

"""# **RESUM:**
### 1.   Objectius:

*   Predir l'alçada del salt amb perxa (height_bar)
*   Analitzar quines variables influeixen més en el rendiment
...
* Entrenats mitjançant train_test_split (80/20)

# OBSERVACIONS:

*   Valorar quines variables són rellevants i quines no

*   Fer una altra predicció amb menys variables ( informació in-situ)

*   Potser s'ha de trovar la manera de que si no introdueixes una variable no   dongui problemas, si no que no la contempli

*   Què passa amb la variable Genero?
...
# 📈 Extreu els noms de les variables i els coeficients
...
# 📊 Crea un DataFrame per ordenar i visualitzar millor
...
plt.xlabel("Pes del coeficient (impacte sobre l'alçada)")
plt.title("🎯 Importància de les variables en el model de regressió lineal")
plt.gca().invert_yaxis()  # la més important a dalt
